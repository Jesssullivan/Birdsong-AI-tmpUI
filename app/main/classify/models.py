...
class Classifier(object):
    ...
    def classify_proc_select(dir=''):
        ...
        # Load in an audio file from user's directory
        audio_fp_raw = glob.glob(dir + '/*')[0]

        try:
            audio_tensor = tfio.audio.AudioIOTensor(audio_fp_raw)
        except:

            err = "Hey! ai.columbari.us cannot yet handle this file without ffmpeg.  \n" + \
                  "Stable implementation using ffmpeg available at: ffmpeg.columbari.us"
            flash(err)
            return err

        num_samples = audio_tensor.shape[0].numpy()
